...
int MPU::initGyro(uint16_t fullScale) {
  ...
  return write(GYRO_SETUP, (regValue<<3)); //set gyro full scale, 00 at 250dps to 11 at 2000dps highest
}
...
int MPU::initAccel(uint8_t fullScale) {
  ...
  return write(ACCEL_SETUP, (regValue<<3)); // set accel full scale, 00 at 2gs, 11 at 16gs
}

int MPU::initMag() {
  int err;
  if(err = write(USR_CNTRL, (1<<5))) return err; //enable i2c master mode for reading from mag

  if(err = write(SLV0_WRITE, (1<<4) | 0b0010)) return err; //prepare data to write to mag internal control register
...
int MPU::readMem(uint16_t addr, uint8_t length, uint8_t *data) {
  ...
  if(err = write(MEM_REG_ADDR, 2, addrBytes)) return err;
  if(err = read(MEM_DATA_ADDR, length, data)) return err;
